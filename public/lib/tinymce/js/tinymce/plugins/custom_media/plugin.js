...
tinymce.PluginManager.add('custom_media', function(editor, url) {
	var urlPatterns = [
		...
		{regex: /player\.youku\.com\/embed\/(\w+)/, type: 'iframe', w: 425, h: 350, url: 'http://player.youku.com/embed/$1'},
		{regex: /http:\/\/v\.youku\.com\/v_show\/id_(\w+)\.html/, type: 'iframe', w: 600, h: 350, url: 'http://player.youku.com/embed/$1'},
	...
	function showDialog() {
        ...
        // if (win.find('#percentage')[0].checked() && width && height && newWidth && newHeight) {
            // newWidth=newWidth+'%';
        // }
			...
			body: [
				{
					...
					items: [
						{name: 'source1', type: 'filepicker', filetype: 'media', size: 40, autofocus: true, label: '优酷视频网址'},
						...
						//{name: 'poster', type: 'filepicker', filetype: 'image', size: 40, label: 'Poster'},
						{
							...
							items: [
								...
								{name: 'percentage', type: 'checkbox', checked: false, text: '使用百分比'}
							]
						}
					]
				}
