...
tinymce.PluginManager.add('custom_autosave', function(editor) {
	...
	function startStoreDraft() {
		if (!started) {
			...
			if(!global) return ;
			if(global.saveInterval) return;
			global.saveInterval=setInterval(function() {
				if(global&&global.change) {
					if(global.autosave) global.autosave(function(){
	...
	settings.autosave_interval = parseTime(settings.autosave_interval, '180s');
