...
define("tinymce/ui/Window", [
	...
	var Window = FloatPanel.extend({
		...
		postRender: function() {
			...
			if(!self.settings.nodrag) this.dragHelper = new DragHelper(self._id + '-dragh', {
				start: function() {
					startPos = {
						x: self.layoutRect().x,
						y: self.layoutRect().y
		...
		remove: function() {
			...
			self._super();
			if(self.dragHelper) self.dragHelper.destroy();
                ...
                onselect: function(e) {
                    if (e.control.settings.value) {
                        ...
                        if(e.control.settings.value=="settings"){
                           EditorManager.activeEditor.execCommand('fonts', false, null);
                        }
                        else {
                            //console.log(e.control.settings);
                            if(e.control.settings.value!="undefined"){
                                EditorManager.activeEditor.execCommand('FontName', false, e.control.settings.value);
                            }
                            else{
                                var text=e.control.settings.text.raw;
                                if(text){
                                    iFonts.show_alert(text);
                                }
                            }
                        }
                    }
		...
		showMenu: function() {
			...
			if (settings.menu) {
                ...
                if(self.settings.value=="收藏字体"){
                    if(typeof iFonts!="undefined"){
                        iFonts.resetTinyMCEFavFonts=function(){
                ...
                if(self.settings.value=="本地字体"){
                    if(typeof iFonts!="undefined"){
                        iFonts.resetTinyMCELocalFonts=function(){
