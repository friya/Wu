...
(function($, undefined) {
    ...
    $.divrotate.getDegreeResizeChange = function(dom, pos, oriPos, oriSize, postype) {
        ...
        var heplerfunc = function(ypos) {
            ...
            height = $.math.sin(bgree) * Math.sqrt(dx * dx + dy * dy);
	...
	_mouseDown: function(event) {
		...
		if (!this.mouseDelayMet) {
			this._mouseDelayTimer = setTimeout(function() {
                ...
                that._iframeFix(event);
			}, this.options.delay);
		}
		
		else{
            this._iframeFix(event);
		}
...
$.widget("ui.draggable", $.ui.mouse, {
	...
	_mouseCapture: function(event) {
        ...
        //we find that iframeFix layout is created before delay and distance ,we want to move it to start
	...
	_mouseStart: function(event) {
		...
		if(o.concernAngle)
			this.offset = this.positionAbs = {top:parseFloat(this.element.css("top"))+this.element.parent().offset().top, left:parseFloat(this.element.css("left"))+this.element.parent().offset().left};
		else
...
$.widget("ui.resizable", $.ui.mouse, {widgetEventPrefix: "resize",options: {alsoResize: false,animate: false,animateDuration: "slow",animateEasing: "swing",aspectRatio: false,aspectFunc: false,autoHide: false,containment: false,ghost: false,grid: false,handles: "e,s,se",helper: false,scroll: false,scrollSensitivity: 20,scrollSpeed: 20,maxHeight: null,maxWidth: null,minHeight: 10,minWidth: 10,zIndex: 1000,concernAngle: false,concernLock: false},_create: function() {
            ...
            var _destroy = function(exp) {
                $(exp).removeClass("ui-resizable ui-resizable-disabled ui-resizable-resizing").removeData("ui-resizable").unbind(".resizable").find(".ui-resizable-handle").remove()
            };
            ...
            if (o.scroll && (!o.concernAngle || o.concernAngle && !this.element.data("deg"))) {
                ...
                this.originalMousePosition.top += this.scrollParent[0].scrollTop
            }
            ...
            if (o.scroll && (!o.concernAngle || o.concernAngle && !this.element.data("deg"))) {
                ypos += this.scrollParent[0].scrollTop
            }
            ...
            var o = this.options, cpos = this.position, csize = this.size, a = this.axis;
            var el = this.element;
            var aspectFunc = o.aspectFunc;
            if (isNumber(data.height)) {
                data.width = aspectFunc(el, data.height, "height")
            } else {
                if (isNumber(data.width)) {
                    data.height = aspectFunc(el, data.width, "width")
                }
            }
            if (a == "sw") {
                data.left = cpos.left + (csize.width - data.width);
